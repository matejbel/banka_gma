...
##        deleteCardButton.config(state='disabled', cursor = 'arrow')
        ...
        if lastPayment != '':
            for i in range(0,len(lastPayment),5):
                datumTransakcie = lastPayment[4+i]
                DD, MM, YYYY = datumTransakcie[:2], datumTransakcie[2:4], datumTransakcie[4:]
                datumTransakcie = f'{DD}/{MM}/{YYYY}'
                print(datumTransakcie)
                sumaTransakcie = lastPayment[2+i] + ' €'
                idUctuPrijemcu = lastPayment[3+i]
...
def loadTransakcie():
    global id_karty, cCTransCardInfo, lastPayment, c, paymentQuantity, pauseLoading, version
